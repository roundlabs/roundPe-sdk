...
export default class RoundPeSDK {
  private _chargeApi;
  private _payoutApi;
  private _utilApi;
  ...
  async getPayout(code: string, options?: any) {
    try {
      const response = await this._payoutApi.getPayout(code, options);
      return response;
    } catch (error) {
      return error;
    }
...
async function testing() {
  ...
  const response1 = await roundpe.getAvailableTokensInfo();
  console.log(response1);
  console.log("==================================");
  
  const response2 = await roundpe.getPayoutAvailableTokensInfo();
  console.log(response2);
  console.log("==================================");
  
  const response3 = await roundpe.getTokenNetworkInfo();
  ...
  // const response3 = await roundpe.getPayoutAvailableTokensInfo();
